...
<?php if( $acadp_query->have_posts() ) : ?>	
    <?php while( $acadp_query->have_posts() ) : $acadp_query->the_post(); $field_meta = get_post_meta( $post->ID ); ?>
		...
		<?php if( $field_meta['type'][0] == 'search_number_from') : 	?>
        		<div class="col-md-12 col-xs-12">
  						<input type="number" name="cf[<?php echo $post->ID; ?> ]" step="<?php echo  $field_meta['numeric_step'][0]; ?>" min="<?php echo  $field_meta['numeric_min'][0]; ?>" max="<?php echo  $field_meta['numeric_max'][0]; ?>" class="form-control" placeholder="<?php _e( 'MIN', 'advanced-classifieds-and-directory-pro' ); ?>" value="<?php if( isset($_GET['cf'][ $post->ID ] ) ) echo esc_attr( $_GET['cf'][ $post->ID ] ); ?>">
            		</div>
        <?php endif; ?> 

		<?php if( $field_meta['type'][0] == 'search_number_to') : 	?>
        		<div class="col-md-12 col-xs-12">
  						<input type="number" name="cf[<?php echo $post->ID; ?> ]" step="<?php echo  $field_meta['numeric_step'][0]; ?>" min="<?php echo  $field_meta['numeric_min'][0]; ?>" max="<?php echo  $field_meta['numeric_max'][0]; ?>" class="form-control" placeholder="<?php _e( 'MAX', 'advanced-classifieds-and-directory-pro' ); ?>" value="<?php if( isset($_GET['cf'][ $post->ID ] ) ) echo esc_attr( $_GET['cf'][ $post->ID ] ); ?>">
            		</div>
        <?php endif; ?> 
