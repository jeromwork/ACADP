...
class ACADP_Public_User_Ajax {
	...
	public function manage_actions() {	
		...
		if( 'POST' == $_SERVER['REQUEST_METHOD'] && isset( $_POST['acadp_crud_nonce'] ) && wp_verify_nonce( $_POST['acadp_crud_nonce'], 'acadp_save_listing' ) ) {
...
$category = $category[0];
	...
	public function ajax_callback_custom_fields( $post_id = 0, $term_id = 0 ) {
		...
		include( acadp_get_template( "user/acadp-public-ajax-custom-fields-display.php" ) );
		wp_reset_postdata(); // Restore global post data stomped by the_post()
		$output = ob_get_clean();
	...
	private function save_listing() {
		...
		if( $post_id ) {
			...
			if( ! empty( $general_settings['has_location'] ) ) {
				...
				$beginLatitude = isset( $_POST['beginLatitude'] ) ? sanitize_text_field( $_POST['beginLatitude'] ) : '';
    			update_post_meta( $post_id, 'beginLatitude', $beginLatitude );
