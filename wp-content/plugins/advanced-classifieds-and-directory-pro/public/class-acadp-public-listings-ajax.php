 ...
 * @access   public
 */
class ACADP_Public_Listings_Ajax {
	...
	public function get_content( $atts) {
		...
		*/
		...
		$current_order = "date-desc";
		if( isset( $_POST['orderby'] ) ) {
			$current_order = sanitize_text_field( $_POST['orderby'] );
		} 
	
		apply_filters( 'acadp_get_listings_current_order', $current_order );
		...
		// Define the query
		$paged = 1;
		if( isset( $_POST['page'] ) ) {			$paged = absint( $_POST['page'] );		}
		else if(isset( $_POST['paged'] )){ 		$paged = absint( $_POST['paged'] );}
...
if(isset($_POST['view']) && $_POST['view'] != ''){
	$current_view = sanitize_text_field( $_POST['view'] );
		...
		if( $acadp_query->have_posts() ) {
			...
			ob_start();
			include( acadp_get_template( "ajax/acadp-public-ajax-display.php" ) );
			wp_die();
			return ob_get_clean();
