(function( $ ) {
	...
	function acadp_add_marker( $marker, map ) {
		...
		// var
		var latlng = new google.maps.LatLng( $marker.data( 'laatitude' ), $marker.data( 'longitude' ) );
		...
		if( map.markers.length ) {
    		for( var i = 0; i < map.markers.length; i++ ) {
        		...
        		if( latlng.equals( pos ) ) {
            		...
            		var laatitude  = latlng.lat() + ( Math.random() - .5 ) / 1500; // * (Math.random() * (max - min) + min);
            		var longitude = latlng.lng() + ( Math.random() - .5 ) / 1500; // * (Math.random() * (max - min) + min);
            		latlng = new google.maps.LatLng( laatitude, longitude );
        		}
    		}
		}
