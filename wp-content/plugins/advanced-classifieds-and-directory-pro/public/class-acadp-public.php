...
class ACADP_Public {
	...
	public function register_enqueue_styles() {
		...
		wp_enqueue_style( ACADP_PLUGIN_NAME, ACADP_PLUGIN_URL.'public/css/acadp-public.css', $deps, ACADP_VERSION_NUM, 'all' );
...
//wp_enqueue_script( 'bootstrap', plugins_url( 'js/bootstrap.min.js', __FILE__ ), array( 'jquery' ), '3.3.5', true );
		...
		//wp_enqueue_script(   ACADP_PLUGIN_NAME.'-yamap', 'https://api-maps.yandex.ru/2.1/?lang=ru_RU&amp;apikey=5525b824-1d06-46cf-8d45-62787ca033aa', array('jquery'), null, true);
		//wp_enqueue_script(   ACADP_PLUGIN_NAME.'-deliveries', ACADP_PLUGIN_URL.'public/js/deliveries.js', array('jquery'), null, true);
		
		
		
		wp_enqueue_script(  ACADP_PLUGIN_NAME.'select2.full', plugins_url( 'js/select2.full.min.js', __FILE__ ), array( 'jquery' ),'', true );
		wp_enqueue_script(  ACADP_PLUGIN_NAME.'select2-custom', plugins_url( 'js/select2-custom.js', __FILE__ ), array(  ACADP_PLUGIN_NAME.'select2.full' ),'', true );
		//wp_enqueue_script( 'bootstrap-growl.min', plugins_url( 'js/bootstrap-growl.min.js', __FILE__ ), array( ACADP_PLUGIN_NAME.'-bootstrap' ) );
		//wp_enqueue_script( 'notification', plugins_url( 'js/notification.js', __FILE__ ), array( 'bootstrap-growl.min' ) );

		wp_enqueue_script(  ACADP_PLUGIN_NAME.'-user', plugins_url( 'js/user.js', __FILE__ ), array( 'jquery' ),null , true);
		wp_enqueue_script(  ACADP_PLUGIN_NAME.'-custom', plugins_url( 'js/custom.js', __FILE__ ), array( 'jquery' ),null , true);
		wp_localize_script( ACADP_PLUGIN_NAME.'-custom', 'ACADP', array(
			'ajaxurl' => admin_url( 'admin-ajax.php' ),
			'name'    => wp_get_current_user()->display_name
		) );
		wp_enqueue_script(  ACADP_PLUGIN_NAME.'-personal_content', plugins_url( 'js/personal_content.js', __FILE__ ), array( 'jquery', ACADP_PLUGIN_NAME.'-custom' ),null , true);
		wp_enqueue_style(  ACADP_PLUGIN_NAME.'-select2.min', plugins_url( 'css/select2.min.css', __FILE__ ), array(),'' );
		wp_enqueue_style( 'notification', plugins_url( 'css/notification.css', __FILE__ ), array(),'' );
		...
		if( is_singular('acadp_listings') ) {
			...
			//wp_enqueue_script( 'custom',  ACADP_PLUGIN_NAME. 'js/custom.js', array( 'jquery' ) );


			wp_enqueue_script(  ACADP_PLUGIN_NAME.'-yamap');
			wp_enqueue_script(  ACADP_PLUGIN_NAME.'-deliveries');
			wp_enqueue_script(  ACADP_PLUGIN_URL.'public/js/test.js', array(), null, true);
