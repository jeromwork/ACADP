...
/*
В данном файле будет рендерится в частности карта, и в ней нужно будет вывести точки начала и конца и путь
Через интерфейс яндекс карт
А все данные возмутся из бд
Не забыть поменять laаtitude
...
*/
	...
	function acadp_add_marker( $marker, map ) {
		...
		// update laatitude and longitude values in the form when marker is moved
	...
	/**
	 * Set the laatitude and longitude values from the address.
     *
	 * @since    1.0.0
	 */
	...
	/**
	 * Update the alaatitude and logitude values in the custom post type "acadp_listings".
     *
	 * @since    1.0.0
	 */
	function acadp_update_latlng( point ) {
		
		$( '#acadp-laatitude' ).val( point.lat() );
